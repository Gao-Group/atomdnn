...
class Data(object):
    ...
    def read_inputdata(self,fp_filename=None, der_filename=None, image_num=None, append=False, read_der=atomdnn.compute_force, verbose=False):

        self.read_fingerprints_from_lmpdump(fp_filename,image_num, append, verbose=verbose)
    ...
    def read_fingerprints_from_lmpdump(self, fp_filename=None, image_num=None, append=False, verbose=False):
        ...
        if '.' in fp_filename and fp_filename.split('.')[-1] == '*':
            ext_marker = '.'
            batch_mode = 1
    ...
    def read_der_from_lmpdump(self, der_filename=None, image_num=None, append=False, verbose=False):
        ...
        while (1):
            if image_num:
                if fd-start_image+1>image_num:
                    break
            if batch_mode:
                filename = xyzfile_path + '/' + xyzfile_name + ext_marker + str(fd)
            else:
            ...
            if os.path.isfile(filename):
                ...
                self.output_dict['pe/atom'].append(pe/self.num_atoms)
                if read_force:

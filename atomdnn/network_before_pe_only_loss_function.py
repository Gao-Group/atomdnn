...
class Network(tf.Module):
    ...
    def loss(self, true_dict, pred_dict, training=False, validation=False, debug=False):
        """
    ...
    def train_step(self, input_dict, output_dict, step=None, verbose=False, debug=False):
        """
        ...
        with tf.GradientTape() as tape: # automatically watch all tensorflow variables 
            ...
            total_loss,loss_dict = self.loss(output_dict, pred_dict, training=True, debug=debug)
    ...
    def train(self, train_dataset, validation_dataset=None, dropout=None, early_stop=None, scaling=None, batch_size=None, epochs=None, loss_fun=None, \
              ...
              shuffle=True, append_loss=False, descriptors=None, folder_id=None, nepochs_checkpoint=None, verbose=False, debug=False):
        """
        ...
        for epoch in range(epochs):
            ...
            for step, (input_dict, output_dict) in enumerate(train_dataset.batch(batch_size)):
                ...
                batch_loss = self.train_step(input_dict, output_dict, step=step, verbose=verbose, debug=debug)
                if step==0:

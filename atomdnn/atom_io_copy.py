...
def create_lmp_input(descriptor=None):
    ...
    compute_fp_line = ''
    compute_der_line = ''
    print('aqui esta el cutoff:', cutoff)
    ...
    lmpinfile.writelines('clear\n'+
                         ...
                         compute_fp_line + 
                         compute_der_line +
                         'dump dump_fingerprints all custom 1 ${dp_filename}.${file_id} id type c_1[*]\n'+
                         'dump_modify dump_fingerprints sort id format float %20.10g\n'+
                         'dump dump_primes all local 1 ${der_filename}.${file_id} c_2[*]\n'+
...
def create_descriptors(xyzfile_path=None, xyzfile_name=None, descriptors_path=None, lmpexe=None, descriptor=None, descriptor_filename='dump_fp', der_filename='dump_der', image_num=None, verbose=False):
    ...
    while (1):
        ...
        if os.path.isfile(filename):    
            ...
            lmp_cmd = lmpexe + ' -in in.gen_descriptors ' + ' -var file_id ' + str(fd+1) + ' -var dp_filename ' + descriptor_filename  + ' -var der_filename ' + der_filename 

...
class Network(tf.Module):
    ...
    def predict(self, input_dict, compute_force=atomdnn.compute_force, training=False, verbose=False, evaluation=False):
        ...
        if not compute_force:
            ...
            if training:
                return {'pe':total_pe, 'pe/atom':tf.reduce_mean(atom_pe, axis=1)} 
            else:
                return {'pe':total_pe.numpy(), 'pe/atom':tf.reduce_mean(atom_pe, axis=1).numpy()}
            ...
            if training:
                print(f'DEBUG1: tf.reduce_mean(atom_pe):{tf.reduce_mean(atom_pe, axis=1)}')
                return {'pe':total_pe, 'pe/atom':tf.reduce_mean(atom_pe, axis=1), 'force':force, 'stress':stress}
            else:
                return {'pe':total_pe.numpy(), 'pe/atom':tf.reduce_mean(atom_pe, axis=1).numpy(), 'force':force.numpy(), 'stress':stress.numpy()}
    ...
    def loss(self, true_dict, pred_dict, training=False, validation=False):
        ...
        tmp_var1 = true_dict['pe/atom']
        tmp_var2 = tf.reshape(pred_dict['pe/atom'], true_dict['pe/atom'].shape)
        print(f'DEBUG2:\n true_dict[pe/atom]:{tmp_var1}\npred_dict[pe/atom]:{tmp_var2}')
        loss_dict['pe_atom_loss'] = self.loss_function(true_dict['pe/atom'], pred_dict['pe/atom']) #ggg
    ...
    def train_step(self, input_dict, output_dict, step=None, verbose=False):
        """
    ...
    def train(self, train_dataset, validation_dataset=None, dropout=None, early_stop=None, scaling=None, batch_size=None, epochs=None, loss_fun=None, \
              optimizer=None, lr=None, decay_lr=False, starting_lr=None, decay_steps=None, decay_power=None, loss_weights=None, compute_all_loss=False, shuffle=True, append_loss=False, descriptors=None, folder_id=None, nepochs_checkpoint=None, verbose=False):
        """
        ...
        for epoch in range(epochs):
            ...
            for step, (input_dict, output_dict) in enumerate(train_dataset.batch(batch_size)):
                ...
                batch_loss = self.train_step(input_dict, output_dict, step=step, verbose=verbose)
                if step==0:
    ...
    def plot_loss(self, start_epoch=1, figsize=(8,5), saveplots=False, folder_id=None, pe_atom_loss_units=None, force_loss_units=None):
        """
        Plot the losses.

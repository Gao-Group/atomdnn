...
def create_lmp_input(descriptor=None):
    ...
    compute_fp_line = 'compute 1 all fingerprints ${cutoff} etaG2 '+ G2_parameters + 'etaG4 ' + G4_parameters + 'zeta ' + zeta_parameters + 'lambda ' + lambda_parameters + 'end\n'
    ...
    while (1):
        if image_num:
            if fd > image_num:
                break
        if batch_mode:
            filename = xyzfile_name + '.' + str(fd+1)
        else:
